/*
  # Complete profiles table setup with RLS policies

  1. New Tables
    - Updates to existing `profiles` table with proper RLS policies
    
  2. Security
    - Enable RLS on `profiles` table
    - Add comprehensive policies for all user roles
    - Add policies for anonymous connection testing
    - Add policies for service role operations
    
  3. Functions
    - `is_system_admin()` - Check if user has system admin role
    - `handle_new_user()` - Automatically create profile on user signup
    - `prevent_role_change()` - Prevent unauthorized role changes
    - `update_updated_at_column()` - Update timestamp on record changes
...
DROP POLICY IF EXISTS "Allow anonymous connection testing" ON profiles;
