/*
  # Fix Supabase RLS Policies and Connection Testing
...
-- Drop all existing policies first (including the one causing the conflict)
DO $$ 
BEGIN
...
CREATE POLICY "Allow anonymous connection testing"
  ...
  FOR SELECT
  TO anon
  USING (true); -- Fixed: Allow anonymous SELECT operations for connection testing
