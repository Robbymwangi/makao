...
BEGIN
  -- Get current user's role from JWT first, then fallback to database
  ...
  IF current_user_role IS NULL AND auth.uid() IS NOT NULL THEN
...
-- Function to check if current user is system admin (improved version)
...
BEGIN
  ...
  -- First try to get role from JWT metadata (more efficient)
  user_role := COALESCE(
    (auth.jwt() -> 'user_metadata' ->> 'role')::text,
    (auth.jwt() -> 'app_metadata' ->> 'role')::text
  );
  
  -- If found in JWT, return result
  IF user_role IS NOT NULL THEN
    RETURN user_role = 'systemAdmin';
  END IF;
  
  -- Fallback: Get user's role from profiles table
