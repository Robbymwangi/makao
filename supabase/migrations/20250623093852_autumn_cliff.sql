/*
  # Create profiles table with authentication integration

  1. New Tables
    - `profiles`
      - `id` (uuid, primary key, references auth.users)
      - `email` (text, unique, not null)
      - `role` (text, default 'user', constrained values)
  ...
  2. Security
    ...
    - Add policy for anonymous users to read profiles

  3. Functions
    - `update_updated_at_column()` - Updates the updated_at timestamp
    - `handle_new_user()` - Creates profile when new user signs up
    - `prevent_role_change()` - Prevents unauthorized role changes
    - `is_system_admin()` - Checks if current user is system admin
